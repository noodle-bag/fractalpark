...
import {
  ...
  type JuliaClassicRegressionRendererProfileV1,
...
interface BrowserCase {
  ...
  readonly profile: JuliaClassicRegressionRendererProfileV1;
...
if (!payloadPath) throw new Error("julia-classic-regression-tier2-v1-worker-payload-missing");
...
function runtimeParameters(
  profile: JuliaClassicRegressionRendererProfileV1,
...
function runCpuTrace(
  ...
  juliaC: readonly [number, number],
  maximumDepth: number = JULIA_CLASSIC_REGRESSION_RENDERER_MAX_DEPTH_V2,
): number[] {
...
function imagePoint(
  ...
  const localX =
    (((x + 0.5) / JULIA_CLASSIC_REGRESSION_RENDERER_IMAGE_WIDTH_V2) * 2 - 1) / profile.view.zoom;
  const localY =
    ((((y + 0.5) / JULIA_CLASSIC_REGRESSION_RENDERER_IMAGE_HEIGHT_V2) * 2 - 1) *
      (JULIA_CLASSIC_REGRESSION_RENDERER_IMAGE_HEIGHT_V2 / JULIA_CLASSIC_REGRESSION_RENDERER_IMAGE_WIDTH_V2)) /
    profile.view.zoom;
...
function runCpuImage(
  ...
  const output: number[] = [];
  for (let y = 0; y < JULIA_CLASSIC_REGRESSION_RENDERER_IMAGE_HEIGHT_V2; y++) {
    for (let x = 0; x < JULIA_CLASSIC_REGRESSION_RENDERER_IMAGE_WIDTH_V2; x++) {
...
function parityShader(
  ...
  binding: JuliaClassicRegressionRendererProfileV1["binding"],
): string {
...
${selectPairFunction("juliaTier2Point", JULIA_CLASSIC_REGRESSION_RENDERER_POINTS_V2)}
${formulaGlsl}
vec4 juliaTier2Run(vec2 point, vec2 orbitC, int requestedSteps) {
  frmV1ResetState(point, orbitC, ${JULIA_CLASSIC_REGRESSION_RENDERER_MAX_DEPTH_V2}, false);
  vec2 orbitZ = initFormula(vec2(0.0), orbitC, point);
  bool keepGoing = !frmV1NonFiniteEvent;
  for (int i = 0; i < ${JULIA_CLASSIC_REGRESSION_RENDERER_MAX_DEPTH_V2}; i++) {
...
void main() {
  ...
  float width = ${JULIA_CLASSIC_REGRESSION_RENDERER_IMAGE_WIDTH_V2}.0;
  float height = ${JULIA_CLASSIC_REGRESSION_RENDERER_IMAGE_HEIGHT_V2}.0;
  ...
  gl_FragColor = juliaTier2Run(point, ${orbitConstant}, ${JULIA_CLASSIC_REGRESSION_RENDERER_IMAGE_ITERATIONS_V2});
}`;
}

function sourceFor(
  runtimeRow: PublishedFormulaRuntimeIndexRowV1,
  correctiveRow: JuliaClassicRegressionCorrectiveRowV1,
): string {
  const baseline = readFileSync(join(RUNTIME_ROOT, runtimeRow.definitionPath), "utf8");
  if (correctiveRow.binding.kind === "source-split") {
    return readFileSync(
      join(ROOT, "resources/formula-library/v1", correctiveRow.candidatePath),
      "utf8",
    );
  }
  return baseline;
...
async function buildCases(ids: readonly string[]): Promise<BrowserCase[]> {
  ...
  const preGpu = parseJuliaClassicRegressionCorrectiveV1(correctiveAsset);
  invariant(preGpu.ok, "julia-classic-regression-tier2-v1-pre-gpu-invalid");
  const runtime = parsePublishedFormulaRuntimeIndexV1(
    JSON.parse(readFileSync(RUNTIME_INDEX_PATH, "utf8")),
  );
  invariant(runtime.ok, "julia-classic-regression-tier2-v1-runtime-index-invalid");
  ...
  for (const formulaId of ids) {
    ...
    invariant(
      preGpuRow?.tier2 === "pending-not-run" &&
        runtimeRow &&
        preGpuRow.candidateSourceRevision.length === 64 &&
        preGpuRow.candidateSemanticHash.length === 64,
      `julia-classic-regression-tier2-v1-row-invalid:${formulaId}`,
    );
    const source = sourceFor(runtimeRow, preGpuRow);
    const adapted = {
      formulaId,
      rowReceipt: preGpuRow.rowReceipt,
      candidateSourceRevision: preGpuRow.candidateSourceRevision,
      candidateSemanticHash: preGpuRow.candidateSemanticHash,
      binding: preGpuRow.binding,
      correctiveBindingRevision: preGpuRow.correctiveBindingRevision,
    ...
    const compiled = await compilePublishedFormulaPluginV1({
      ...
      sourceRevision: adapted.candidateSourceRevision,
      semanticHash: adapted.candidateSemanticHash,
      source,
    });
    invariant(compiled.ok, `julia-classic-regression-tier2-v1-compile-failed:${formulaId}`);
    const { profile, profileDigest } = buildJuliaClassicRegressionRendererProfileV1(
      runtimeRow,
      adapted,
    );
    const traceExpected: number[] = [];
    for (const constant of JULIA_CLASSIC_REGRESSION_RENDERER_CONSTANTS_V2)
      for (const point of JULIA_CLASSIC_REGRESSION_RENDERER_POINTS_V2)
        traceExpected.push(
    ...
    const integrationWitness =
      formulaId === JULIA_CLASSIC_REGRESSION_RENDERER_INTEGRATION_WITNESS_FORMULA_ID_V2;
    ...
    invariant(
      ...
      `julia-classic-regression-tier2-v1-parameter-uniform-missing:${formulaId}`,
    );
    cases.push({
      formulaId,
      candidateContentHash: preGpuRow.rowReceipt,
      sourceRevision: adapted.candidateSourceRevision,
      semanticHash: adapted.candidateSemanticHash,
      binding: profile.binding,
      bindingRevision: preGpuRow.correctiveBindingRevision,
      supportLane: preGpuRow.supportLane,
      profileDigest,
      profile,
      bindingUniformName,
      ...
      imageExpectedA: runCpuImage(
        ...
        profile,
        JULIA_CLASSIC_REGRESSION_RENDERER_CONSTANTS_V2[0],
      ),
      imageExpectedB: runCpuImage(
        compiled.value.backend,
        profile,
        JULIA_CLASSIC_REGRESSION_RENDERER_CONSTANTS_V2[1],
      ),
    });
  }
  return cases;
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const results = await page.evaluate(async (browserCases) => {
      ...
      const output: JuliaClassicRegressionRendererReportRowV1[] = [];
