...
const fail = (code: string): never => { throw new Error(`julia-pre-gpu-recovery:${code}`); };
...
void build()
  .then(({ census, manifest }) => {
    ...
    if (process.argv.includes("--write")) {
      writeFileSync(`${OUTPUT}.tmp`, censusBytes, { mode: 0o644 });
      writeFileSync(`${MANIFEST_OUTPUT}.tmp`, manifestBytes, { mode: 0o644 });
      renameSync(`${OUTPUT}.tmp`, OUTPUT);
      renameSync(`${MANIFEST_OUTPUT}.tmp`, MANIFEST_OUTPUT);
    }
    if (
      !existsSync(OUTPUT) ||
