...
if (!payloadPath) throw new Error("julia-failure-diagnostic-payload-missing");
const DIAGNOSTIC_TARGET_IDS = Object.freeze(
  rendererAsset.rows
...
if (
  DIAGNOSTIC_TARGET_IDS.length !== 15 ||
  new Set(DIAGNOSTIC_TARGET_IDS).size !== 15
)
  throw new Error("julia-failure-diagnostic-target-set-invalid");
...
async function buildCases(ids: readonly string[]): Promise<BrowserCase[]> {
  ...
  for (const formulaId of ids) {
    ...
    invariant(
      preGpuRow &&
        runtimeRow &&
        (DIAGNOSTIC_TARGET_IDS as readonly string[]).includes(formulaId),
      `julia-failure-diagnostic-row-invalid:${formulaId}`,
    );
    invariant(
    ...
    invariant(parsedSource.ok, `julia-failure-diagnostic-source-parse:${formulaId}`);
    const revision = await hashFrmLikeV1(source, parsedSource.ir);
    invariant(
      revision.sourceRevision === preGpuRow.evaluatedSourceRevision &&
        revision.semanticHash === preGpuRow.evaluatedSemanticHash,
      `julia-failure-diagnostic-source-binding:${formulaId}`,
    );
...
async function main(): Promise<void> {
  ...
  invariant(
    Array.isArray(payload.ids) &&
      payload.ids.length === DIAGNOSTIC_TARGET_IDS.length &&
      new Set(payload.ids).size === DIAGNOSTIC_TARGET_IDS.length &&
      [...payload.ids].sort().every(
        (formulaId, index) => formulaId === DIAGNOSTIC_TARGET_IDS[index],
      ),
    "julia-failure-diagnostic-payload-invalid",
  );
  ...
  try {
    ...
    const results = await page.evaluate(async (browserCases) => {
      ...
      const diagnosticFailureCode = (error: unknown): string => {
        const message = error instanceof Error ? error.message : "renderer-row-failed";
        const indexed = /^(trace-state-mismatch|trace-flag-mismatch):(\d+)$/.exec(
          message,
        );
        if (indexed) return `${indexed[1]}:${indexed[2]}`;
        const imageIndexed = /^(image-state-mismatch|image-flag-mismatch):([AB]):(\d+)$/.exec(
          message,
        );
        if (imageIndexed)
          return `${imageIndexed[1]}:${imageIndexed[2]}:${imageIndexed[3]}`;
        if (
          [
          ...
          return message;
        if (message.startsWith("full-framework"))
          return "full-framework-probe-failed";
        return "shader-compile-or-link-failed";
      };
      ...
      for (const testCase of browserCases) {
        ...
        try {
            ...
            for (let index = 0; index < actual.length; index++) {
              ...
              if (index % 4 >= 2) {
                if (actualValue >= 0.5 !== expectedValue >= 0.5)
                  throw new Error(`image-flag-mismatch:${label}:${index}`);
              } else {
                maximumRelativeError = Math.max(
                  maximumRelativeError,
                ...
                if (!close(actualValue, expectedValue))
                  throw new Error(`image-state-mismatch:${label}:${index}`);
              }
            }
          };
          compareImage("A", imageA, testCase.imageExpectedA);
          compareImage("B", imageB, testCase.imageExpectedB);
          ...
          output.push({
            ...
            profileDigest: testCase.profileDigest,
            status: "blocked",
            reasonCode: diagnosticFailureCode(error),
            rendererClass: "SwiftShader-software",
