...
import { constants, chmodSync, closeSync, existsSync, fstatSync, lstatSync, mkdirSync, mkdtempSync, openSync, readFileSync, realpathSync, rmSync, writeFileSync } from "node:fs";
...
const fail = (code: string): never => { throw new Error(`julia-failure-diagnosis:${code}`); };
...
function ensurePrivateDirectory(path: string): void {
  if (!existsSync(path)) {
    if (!WRITE) fail("private-output-missing");
...
function writeOrVerify(path: string, bytes: string): void {
  if (existsSync(path)) {
    const status = lstatSync(path);
    if (!status.isFile() || status.isSymbolicLink() || status.nlink !== 1 || (status.mode & 0o777) !== 0o600 || readFileSync(path, "utf8") !== bytes) fail("private-artifact-drift");
    return;
  }
  if (!WRITE) fail("private-artifact-missing");
  ...
  try {
    ...
    if (!status.isFile() || status.nlink !== 1 || (status.mode & 0o777) !== 0o600) fail("private-artifact-write-invalid");
...
async function main(): Promise<void> {
  ...
  privateOutputReady();
  for (const [hash, bytes] of receiptBytes) writeOrVerify(join(RECEIPTS, `${hash}.json`), bytes);
  writeOrVerify(join(MANIFESTS, `${manifestHash}.json`), `${JSON.stringify(manifest, null, 2)}\n`);
  process.stdout.write(`${JSON.stringify({ ok: true, manifestContentHash: manifestHash, receiptCount: receiptRows.length, cpuCount: 8, rendererCount: 15 })}\n`);
}
