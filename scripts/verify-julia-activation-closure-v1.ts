...
function main(): void {
  ...
  assert(exact(acknowledgment, [
    ...
    acknowledgment.approvedAt === "2026-09-01T16:37:15+08:00" &&
  ...
  assert(acknowledgment.approvalStatement ===
    "确认 current-candidate 完整 source-closed 证据链重建后 exact-195 supported 与 exact-11 residual 的成员、状态和 digest 均未改变；仅重绑当前证据链，不授权执行激活、Ready、merge、部署或 Release。" &&
    acknowledgment.residualDispositionResponse === "不弄了，跳过这11个" &&
    acknowledgment.maintainerResponse ===
      "签发更正后的 v2 acknowledgment：接受相同 exact-195 / exact-11 边界，仅重绑当前完整 source-closed 证据链；仍不授权激活、Ready、merge、部署或 Release" &&
    same(acknowledgment.trustModel, trustModel) &&
    acknowledgment.aiAssistanceDisclosure ===
      "This corrected source-rebound acknowledgment receipt and its machine closure were drafted with AI assistance by Ellie; the unchanged exact-195/exact-11 boundary and the limited full-chain rebind scope were explicitly approved by the human maintainer.",
  "ack-decision");
