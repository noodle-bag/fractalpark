...
--    p_formula_id null + p_expected_revision null -> create (quota-checked);
--    otherwise update with optimistic concurrency. The per-owner advisory
--    lock (seed 0, shared with account deletion) serializes writes against
--    the deletion transaction, closing the check-then-act race.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if p_formula_id is null then
    ...
    insert into public.custom_formulas (owner_id, name, source, experience_hint, source_bytes)
    values (p_owner_id, p_name, p_source, p_experience_hint, octet_length(p_source))
    ...
    if p_expected_revision is null or v_formula.revision <> p_expected_revision then
...
-- ---------------------------------------------------------------------------
-- 9. Grants: RPCs are service-role only, mirroring the draft RPC posture.
-- ---------------------------------------------------------------------------
